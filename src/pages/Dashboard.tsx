...
export default function Dashboard() {
  ...
  // Add: Investment calculator state
  const [invAmount, setInvAmount] = useState<number>(100);
      ...
      <main className="container mx-auto px-4 py-8 relative">
        <Tabs defaultValue="challenges" className="space-y-6">
          <TabsList className="bg-gray-900/50 border border-cyan-400/30">
            ...
            <TabsTrigger value="investments" className="data-[state=active]:bg-cyan-400/20 data-[state=active]:text-cyan-400">
              💹 Investments
            </TabsTrigger>
          </TabsList>
            ...
            <Card className="bg-gray-900/50 border-cyan-400/30">
              ...
              <CardContent className="space-y-4">
                {(() => {
                  ...
                  // Add: holdings & buy/sell hooks
                  // eslint-disable-next-line react-hooks/rules-of-hooks
                  const holding = useQuery(api.market.getHolding);
                  // eslint-disable-next-line react-hooks/rules-of-hooks
                  const buy = useMutation(api.market.buyProperty);
                  // eslint-disable-next-line react-hooks/rules-of-hooks
                  const sell = useMutation(api.market.sellProperty);
                  ...
                  // Holdings computed values
                  const units = holding?.units ?? 0;
                  const totalCost = holding?.totalCost ?? 0;
                  const currentValue = last ? Math.floor(units * last) : 0;
                  const pl = currentValue - Math.floor(totalCost);
                  const plPct = totalCost > 0 ? ((pl / totalCost) * 100).toFixed(2) : "0.00";
                  const avgCost = units > 0 ? (totalCost / units).toFixed(2) : "—";
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      {/* Buy / Sell Controls and Holdings */}
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div className="p-4 bg-gray-800/40 rounded border border-gray-700 space-y-3 lg:col-span-2">
                          <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-300 font-semibold">Your Holdings</div>
                            <div className="text-xs text-gray-500">
                              Price per unit: <span className="text-cyan-400">{last || 0} CR</span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                            <div className="p-2 bg-gray-900/60 rounded border border-gray-700">
                              ...
                              <div className="text-cyan-400 font-bold">{units.toFixed(4)}</div>
                            </div>
                            <div className="p-2 bg-gray-900/60 rounded border border-gray-700">
                              <div className="text-gray-400">Avg Cost</div>
                              <div className="text-gray-300 font-bold">{avgCost} CR</div>
                            </div>
                            <div className="p-2 bg-gray-900/60 rounded border border-gray-700">
                              <div className="text-gray-400">Current Value</div>
                              <div className="text-cyan-400 font-bold">{currentValue} CR</div>
                            </div>
                            <div className="p-2 bg-gray-900/60 rounded border border-gray-700">
                        ...
                        <div className="p-4 bg-gray-800/40 rounded border border-gray-700 space-y-4">
                          <div className="text-sm text-gray-300 font-semibold">Trade</div>
                          <div className="space-y-2">
                            <Label className="text-cyan-400">Buy (by credits)</Label>
                            <div className="flex items-center gap-2">
                              <Input
                                type="number"
                                min={1}
                              ...
                              <Button
                                className="bg-green-500/20 border border-green-500 text-green-400 hover:bg-green-500/30"
                                disabled={!buyAmount || buyAmount <= 0 || !last}
                                onClick={async () => {
                                  try {
                                    await buy({ amount: buyAmount } as any);
                                    toast.success("Purchased units");
                                    setBuyAmount(0);
                                  } catch (e) {
                          ...
                          <div className="space-y-2">
                            <Label className="text-cyan-400">Sell (by units)</Label>
                            <div className="flex items-center gap-2">
                              <Input
                                type="number"
                                min={0}
                                step="0.0001"
                                value={sellUnits || ""}
                                onChange={(e) => setSellUnits(parseFloat(e.target.value) || 0)}
                                placeholder="Units"
                                className="bg-gray-800 border-gray-600 text-white"
                              />
                              <Button
                                variant="outline"
                                className="border-red-500 text-red-500 hover:bg-red-500/10"
                                disabled={!sellUnits || sellUnits <= 0 || sellUnits > units || !last}
                                onClick={async () => {
                                  try {
                                    await sell({ units: sellUnits } as any);
                                    toast.success("Sold units");
                                    setSellUnits(0);
                                  } catch (e) {
